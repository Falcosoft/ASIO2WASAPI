...
const char * szSampRateVSTDriverRegValName = "SampleRate";
...
void ASIO2WASAPI::readFromRegistry()
{
    ...
    if (ERROR_SUCCESS == lResult)
    {
        ...
        RegGetValue(key, NULL, szSampRateVSTDriverRegValName, RRF_RT_REG_DWORD, NULL, &m_nSampleRate, &size);
        RegCloseKey(key);
        if (m_nBufferSize > 50) m_nBufferSize = 0;
...
#ifdef _WIN64           
            SetWindowText(hwndDlg, "VST Driver - ASIO2WASAPI x64");
#else	
...
{    
    ...
    HWND parentWindow;
    if (m_hAppWindowHandle)
        parentWindow = m_hAppWindowHandle;
    else
...
ASIOBool ASIO2WASAPI::init(void* sysRef)
{
    ...
    {        
        ...
        else
        {
            ...
            if (FAILED(hr))
                return false;
            m_bufferSize = (int)ceil(hnsDefaultPeriod * 0.0001 * m_nSampleRate * 0.001); // with ceil() we can also handle 49716 Hz
        }
    }    
